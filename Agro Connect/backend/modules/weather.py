...
import requests
from modules.variables import city, weather_api

BASE_URL = "http://dataservice.accuweather.com"

def get_city_key():
    """Fetch city key from AccuWeather using the city name."""
    url = f"{BASE_URL}/locations/v1/cities/search?apikey={weather_api}&q={city}"
...
def get_weather_data():
    ...
    url = f"{BASE_URL}/currentconditions/v1/{city_key}?apikey={weather_api}&details=true"
    response = requests.get(url)
